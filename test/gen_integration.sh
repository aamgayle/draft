    ...
    steps:
      ...
      - name: Curl Endpoint
        run: |
          ...
          kill \$tunnelPID
      - run: ./draft -b main -v generate-workflow -d ./langtest/ -c someAksCluster -r someRegistry -g someResourceGroup --container-name someContainer -x .
    ...
    steps:
      ...
      - name: Curl Endpoint
        run: |
          ...
          curl -m 3 \$SERVICEIP:$serviceport
          kill \$tunnelPID
      - run: ./draft -v generate-workflow -b main -d ./langtest/ -c someAksCluster -r someRegistry -g someResourceGroup --container-name someContainer -x .
      - name: Execute dry run for update command
        run: |
          mkdir -p test/temp
          ./draft --dry-run --dry-run-file test/temp/update_dry_run.json update -d ./langtest/ $ingress_test_args  
    ...
    steps:
      ...
      - name: Curl Endpoint
        run: |
          ...
          curl -m 3 \$SERVICEIP:$serviceport
          kill \$tunnelPID
      - run: ./draft -v generate-workflow -d ./langtest/ -b main -c someAksCluster -r localhost -g someResourceGroup --container-name testapp -x .
      - uses: actions/upload-artifact@v3
        with:
